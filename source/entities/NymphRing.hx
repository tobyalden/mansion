...
class NymphRing extends Entity {
    ...
    public function new(nymph:Nymph) {
        super();
        this.nymph = nymph;
        type = "hazard";
    ...
    public function returnToNymph(newReturnTime:Float = RETURN_TIME) {
        isChasing = false;
        isBouncing = false;
        isEnrageTossed = false;
        isReturning = true;
    ...
    override public function update() {
        ...
        if(isEnrageTossed) {
            ...
            moveTo(
                nymph.x + nymph.width / 2 - width / 2 + velocity.x + 12,
                y
            );
            if(
                top >
                nymph.screenCenter.y + nymph.height / 2
                + GameScene.PLAYFIELD_SIZE / 2
            ) {
                y -= (GameScene.PLAYFIELD_SIZE + height);
                velocity = new Vector2(0, 0);
                returnToNymph(ENRAGE_RETURN_TIME);
            }
        }
        else if(isBouncing) {
            var collideTypes = ["walls"];
            var collideTypes = (
                collideWith(nymph, x, y) != null ?
                ["walls"] : ["walls", "enemy"]
            );
            moveBy(
            ...
            if(velocity.length > 0) {
                ...
                if(!returnTween.active) {
                    returnTween.setMotion(
                        x, y,
                        nymph.centerX - width / 2,
                        nymph.centerY - height / 2,
                        returnTime,
                        Ease.sineInOut
                    );
                    returnTween.start();
